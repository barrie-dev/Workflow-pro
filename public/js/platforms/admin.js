...
<div class="adm-card" style="overflow:auto">
  ...
  // Gedeelde helpers voor de uitgesplitste schermmodules. Ze BLIJVEN hier
  // wonen (admin.js gebruikt ze zelf ook) · dit is exposeren, geen verhuizen.
  // Kopiëren zou twee waarheden opleveren die uiteen gaan lopen.
  A.tA = tA;
  A.uName = uName;
  A.empNameById = empNameById;
  A.uiConfirm = uiConfirm;
  A.uiInput = uiInput;
  A.navFlyoutGo = navFlyoutGo;
  A.openLeaveReviewModal = openLeaveReviewModal;
  // LET OP: A.drawers.workorder is hier GEEN vervanger voor. Die is
  // gedefinieerd als prefill => openWorkorderDrawer(null, null, prefill) en
  // gooit dus zowel de werkbon als de voorgeladen lijst weg. Een scherm dat
  // een aangeklikte rij opent heeft de rauwe functie nodig; via de drawer-
  // variant zou er stil een LEEG paneel opengaan.
  A.openWorkorderDrawer = openWorkorderDrawer;
  A.tWoStatus = tWoStatus;
  A.tLeaveType = tLeaveType;
  A.tLeaveStatus = tLeaveStatus;
  A.views = A.views || {};
  A.drawers = A.drawers || {};

  // Kern-renderers registreren (worden gaandeweg naar modules verplaatst).
  //
  // De elf uitgesplitste schermen staan hier NIET meer: dashboard, actions,
  // employees, planning, appointments, incidents, inbox, leaves, expenses,
  // workorders en messages registreren zichzelf vanuit hun eigen bestand.
  // Een achtergebleven sleutel zou hier een ReferenceError geven tijdens het
  // evalueren van deze IIFE · en dan valt niet één scherm uit maar de HELE
  // admin-shell, want wfp_adminInit wordt dan nooit gezet.
  Object.assign(A.views, {
    operations: renderOperationsOverview,
  ...
  // Alleen de drawers van schermen die HIER nog wonen. De uitgesplitste
  // modules registreren hun eigen drawer (employee, message, shift,
  // appointment, incident, inquiry).
  //
  // Een achtergebleven verwijzing naar een verhuisde functie is geen kleine
  // fout: `employee: openEmployeeDrawer` wordt bij het laden geëvalueerd en
  // geeft dan een ReferenceError, waardoor deze hele IIFE afbreekt en
  // wfp_adminInit nooit gezet wordt. Het gevolg is een lege applicatie, niet
  // een ontbrekend paneel · en `node --check` ziet dat niet.
  Object.assign(A.drawers, {
    customer: openCustomerDrawer,
